...
The Fast Fourier Transform is one of the most important algorithms in computing. It reduces the cost of computing a Discrete Fourier Transform from $O(n^2)$ to $O(n \log n)$~\cite{cooley1965}, and it appears everywhere from signal processing to polynomial multiplication to data compression.
...
The parser takes the token stream and builds an Abstract Syntax Tree. It is written using \texttt{funcparserlib}~\cite{funcparserlib}, a combinator library, with manual operator precedence handling. Each level of precedence is defined as a rule that folds operators left-to-right, correctly expressing left-associativity:
...
The interpreter walks the AST and evaluates each node. This is the defining characteristic of the \textit{interpreted} paradigm adopted by Seralix: rather than compiling the program to a lower-level representation~\cite{aho_compilers}, execution proceeds directly over the tree structure produced by the parser. It maintains a stack of variable environments, one per function call, so that local variables in one function do not interfere with another.
...
Seralix supports both \texttt{while} and \texttt{for-in} loops:
...
where $\omega_n = e^{2\pi i / n}$ is a complex root of unity. Computing this directly for every $k$ costs $O(n^2)$ multiplications~\cite{clrs}.
...
satisfies $\omega_n^n \equiv 1 \pmod{p}$, which is all the butterfly needs. This project uses $p = 998244353$ with primitive root $g = 3$, a standard choice in competitive programming~\cite{ntt_programming} that supports transform sizes up to $2^{23}$.

\subsection{Gaussian Integer FFT: Exact Complex Version}

For cases where actual complex structure matters, this project also implements an FFT over Gaussian integers modulo a prime~\cite{gaussian_fft}. A Gaussian integer is a complex number $a + bi$ where both $a$ and $b$ are integers. When working modulo a prime $p$ where $p \equiv 1 \pmod{4}$, a square root of $-1$ exists modulo $p$, which means we can represent $i$ exactly as an integer. Complex numbers then become pairs of integers $[re, im]$, and all arithmetic stays exact:
...
All six variants — NTT standard, NTT with $n=8$, NTT with $n=16$, Gaussian FFT standard, Gaussian FFT with $n=8$, Gaussian FFT with $n=16$ — produce identical output, cross-validated against the na\"{i}ve $O(n^2)$ baseline.
...
\begin{itemize}
    \item \textbf{Recursion}: the recursive FFT calls itself on even and odd subsequences without issue across all tested input sizes.
    \item \textbf{List indexing and assignment}: bit-reversal permutation and butterfly updates require in-place list writes by index, all of which execute correctly.
    \item \textbf{Bitwise arithmetic}: the bit-reversal step uses left shift, right shift, and bitwise OR; all produce correct results.
    \item \textbf{Nested loops}: the three-level butterfly loop (over stages, groups, and positions) executes without error.
    \item \textbf{Closures}: the NTT and FFT functions reference module-level constants (\texttt{MOD}, \texttt{PRIM\_ROOT}) captured at definition time, which are correctly resolved at call time.
    \item \textbf{Higher-order behaviour}: helper functions such as \texttt{modpow} and \texttt{modinv\_prime} are passed around and called correctly as first-class values.
\end{itemize}
...
\subsection{Floating-Point Limitation Was Overcome}
...
\begin{itemize}
    \item \textbf{List append performance}: adding an element to a list via \texttt{result + [x]} copies the entire list each time, making repeated appends $O(n^2)$ in the interpreter. For the small inputs tested this is not observable, but it becomes the dominant cost for large polynomial degrees.
    \item \textbf{No caching}: functions like the cyclotomic polynomial builder recompute their result on every call, even when the input has not changed. Memoisation would require a native dictionary type at module scope.
    \item \textbf{No type checking}: passing a non-prime as a modulus, for example, produces arithmetically incorrect output rather than an early error. There is no mechanism to detect this class of mistake before execution.
    \item \textbf{Stack depth}: deeply recursive calls are bounded by Python's own recursion limit (default 1000 frames). For the recursive FFT this becomes critical at input sizes of $n = 2^{10} = 1024$ and above, where the recursion depth equals $\log_2 n = 10$ levels but each level spawns two sub-calls, exhausting the stack rapidly. The iterative NTT avoids this entirely; the recursive FFT is therefore suitable only for small inputs or after raising Python's recursion limit explicitly.
\end{itemize}
...
Cambridge, MA: MIT Press, 2022, ch.~30.

\bibitem{ntt_ref}
D.~Harvey and J.~van~der~Hoeven,
``Integer multiplication in time $O(n \log n)$,''
\textit{Annals of Mathematics}, vol.~193, no.~2, pp.~563--617, 2021.

\bibitem{gaussian_fft}
R.~Crandall and B.~Fagin,
``Discrete weighted transforms and large-integer arithmetic,''
\textit{Mathematics of Computation}, vol.~62, no.~205, pp.~305--324, 1994.

\bibitem{ntt_programming}
A.~Laaksonen,
\textit{Competitive Programmer's Handbook}.
Self-published, 2018, ch.~30 (Number Theory), available online.

\bibitem{funcparserlib}
V.~Vlasov,
\texttt{funcparserlib}: Recursive descent parser library for Python,
version 1.0.1, 2022.
[Online]. Available: \url{https://github.com/vlasovskikh/funcparserlib}

\bibitem{aho_compilers}
